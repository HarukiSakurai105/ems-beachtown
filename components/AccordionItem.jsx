  ...
  return <article id={rule.id} className={clsx('overflow-hidden rounded-[1.6rem] border bg-[var(--panel)]', open ? 'border-[var(--line-strong)] shadow-[var(--shadow-soft)]' : 'border-[var(--line)]')}>
    <div className="flex items-stretch">
      <button onClick={toggle} aria-expanded={open} className="flex min-w-0 flex-1 items-center gap-3 p-4 text-left sm:gap-4 sm:p-5">
        ...
        <span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><small className="text-[9px] font-black uppercase tracking-[.16em] text-[#ff5d45]">{rule.num}</small>{isNewRule && <small className="inline-flex items-center gap-1 rounded-full bg-[#ff5d45] px-2 py-1 text-[8px] font-black uppercase text-white"><Sparkles className="h-2.5 w-2.5" /> Mới</small>}</span><b className="mt-1 block text-sm leading-5 text-[var(--ink)] sm:text-base">{rule.title}</b></span>
        <span className="hidden text-[10px] font-bold text-[var(--muted)] sm:block">{rule.items?.length || 0} mục</span>
